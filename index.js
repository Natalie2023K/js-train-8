...
function createArray(...elements) {
  return elements.length > 0 ? Array.of(elements) : 'Помилка';
...
function findElementIndexes(arr, element) {
  ...
  ? Array.of(arr.indexOf(element), arr.lastIndexOf(element))
  : [];
...
function copyAndSwapElements(arr, target, start, end) {
  return arr.length >= target && arr.length >= start && arr.length >= end
...
function customEvery(arr, condition) {
  return typeof condition === `function` && Array.isArray(arr) 
  ? arr.every(condition)
  : 'false';
...
function customFill(arr, value, start = 0, end = arr.length) {
  return Array.isArray(arr) && arr.length >= start && arr.length > end
...
function customAt(arr, index) { 
  if(!Array.isArray(arr) &&  index >= arr.length) {
   return undefined;
    ...
    console.log('Елемент є рядком,числом')
...
function customIncludes(arr, element) {
  if(Array.isArray(arr)) {
    let custInc = arr.includes(element);
